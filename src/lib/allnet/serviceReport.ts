...
/** מחשב שעות עבודה מדווחות עבור קריאה (לפי שעת התחלה/סיום) */
function callHours(c: ServiceCall): number {
  if (!c.workFrom || !c.workTo) return 0;
  const [fh, fm] = c.workFrom.split(":").map(Number);
  const [th, tm] = c.workTo.split(":").map(Number);
  if ([fh, fm, th, tm].some((n) => Number.isNaN(n))) return 0;
  const diff = (th! * 60 + tm!) - (fh! * 60 + fm!);
  return diff > 0 ? Math.round((diff / 60) * 100) / 100 : 0;
}

const fmtH = (n: number) => `${Math.round(n * 100) / 100} ש'`;

/**
 * דוח מנהלי מרוכז של כל קריאות השירות — פילוח סטטוסים, פירוט מלא וסיכום שעות מדווחות.
 */
export function openServiceCallsOverviewReport(
  calls: ServiceCall[],
  technicianName: (username?: string) => string,
) {
  if (!calls.length) return false;
  ...
  const byStatus = (["new", "assigned", "in_progress", "done"] as const).map((s) => ({
    status: s,
    label: SERVICE_STATUS_LABELS[s],
    count: sorted.filter((c) => c.status === s).length,
    hours: sorted.filter((c) => c.status === s).reduce((a, c) => a + callHours(c), 0),
  }));

  const totalHours = sorted.reduce((a, c) => a + callHours(c), 0);
  const reported = sorted.filter((c) => callHours(c) > 0).length;

  const techMap = new Map<string, { count: number; hours: number }>();
  for (const c of sorted) {
    const name = technicianName(c.technician) || "ללא שיוך";
    const cur = techMap.get(name) ?? { count: 0, hours: 0 };
    techMap.set(name, { count: cur.count + 1, hours: cur.hours + callHours(c) });
  }

  const statusCards = byStatus
    .map(
      (s) =>
        `<div class="card"><div class="k">${esc(s.label)}</div><div class="v">${s.count} · ${fmtH(s.hours)}</div></div>`,
    )
    .join("");
  ...
  const detailRows = sorted
    .map((c) => {
      ...
      return `<tr>
        ...
        <td>${esc(c.subject) || "—"}</td>
        <td>${formatDateIL(c.createdAt)}</td>
        <td>${c.closedAt ? formatDateIL(c.closedAt) : "—"}</td>
        <td>${esc(technicianName(c.technician)) || "—"}</td>
        <td>${badge(SERVICE_STATUS_LABELS[c.status], statusC)}</td>
        <td>${c.workFrom && c.workTo ? `${esc(c.workFrom)}-${esc(c.workTo)}` : "—"}</td>
        <td><b>${callHours(c) ? fmtH(callHours(c)) : "—"}</b></td>
      </tr>`;
    })
    .join("");
  ...
  const techRows = Array.from(techMap.entries())
    ...
    .map(
      ([name, v]) =>
        `<tr><td>${esc(name)}</td><td>${v.count}</td><td><b>${fmtH(v.hours)}</b></td></tr>`,
    )
    .join("");
  ...
  <div class="sheet">
    <div class="top">
      <img src="${logoUrl}" alt="AllNet" />
      <div class="meta">
        <h1>דוח כללי — מערכת קריאות שירות</h1>
        <div class="sub">${sorted.length} קריאות · ${reported} קריאות עם דיווח שעות · סה"כ ${fmtH(totalHours)} · הופק בתאריך ${formatDateIL(new Date().toISOString())}</div>
      </div>
    </div>
    <div class="content">
      <h2>פילוח לפי סטטוס</h2>
      <div class="cards" style="grid-template-columns: repeat(4, 1fr)">${statusCards}</div>

      <h2>סיכום שעות לפי טכנאי</h2>
      <table class="summary">
        <thead><tr><th>טכנאי</th><th>מס' קריאות</th><th>סה"כ שעות מדווחות</th></tr></thead>
        <tbody>${techRows}</tbody>
      </table>

      <h2>פירוט כל הקריאות</h2>
      <table class="summary">
        <thead>
          <tr>
            <th>מס'</th><th>לקוח</th><th>אתר</th><th>נושא</th><th>נפתחה</th><th>נסגרה</th>
            <th>טכנאי</th><th>סטטוס</th><th>שעות עבודה</th><th>סה"כ</th>
          </tr>
        </thead>
        <tbody>${detailRows}</tbody>
        <tfoot>
          <tr><td colspan="9"><b>סה"כ שעות מדווחות</b></td><td><b>${fmtH(totalHours)}</b></td></tr>
        </tfoot>
      </table>
    </div>
    <footer>
      <span>AllNet · דוח כללי קריאות שירות</span>
      <span>מסמך זה הופק ממערכת הניהול והתפעול של AllNet</span>
    </footer>
  </div>`;

  return openPrintWindow(`דוח כללי קריאות שירות - ${sorted.length} קריאות`, html);
}
